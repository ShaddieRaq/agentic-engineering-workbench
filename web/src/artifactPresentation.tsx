...
import { ErrorNotice, JsonView, RawDrawer, StatusBadge } from "./components.js";
...
export function ArtifactPresentationView({
  ...
  const reportLabel = presentation.presentationKind === "documentation-audit"
    ...
    : presentation.presentationKind === "playwright-failure-triage"
      ? "Triage"
      : presentation.presentationKind === "agent-improvement"
        ? "Improvement"
      : "Evidence";
  return <>
    ...
    {presentation.improvement?.proposal && <section><div className="section-heading"><div><span className="eyebrow">Cited diagnosis</span><h2>Failure modes</h2></div><StatusBadge value={presentation.improvement.proposal.disposition} /></div><div className="finding-list">{presentation.improvement.proposal.failureModes.map((failure) => <article className={`finding-card severity-${failure.confidence}`} key={failure.title}><div className="finding-heading"><h3>{failure.title}</h3><span className={`severity severity-${failure.confidence}`}>{failure.confidence} confidence</span></div><p>{failure.explanation}</p><div className="tag-row">{failure.evidenceIds.map((id) => <code className="tag" key={id}>{id}</code>)}</div></article>)}</div></section>}
    {presentation.improvement?.proposal && <section><div className="section-heading"><div><span className="eyebrow">Reviewable changes</span><h2>Recommendations</h2></div><span>{presentation.improvement.proposal.recommendations.length} total</span></div><div className="finding-list">{presentation.improvement.proposal.recommendations.map((recommendation) => <article className="detail-block" key={`${recommendation.category}-${recommendation.title}`}><div className="finding-heading"><div><span className="eyebrow">{recommendation.category}</span><h3>{recommendation.title}</h3></div><StatusBadge value={recommendation.priority} /></div><p>{recommendation.rationale}</p><div className="recommendation"><strong>Proposed change</strong><p>{recommendation.proposedChange}</p></div><div className="tag-row">{recommendation.evidenceIds.map((id) => <code className="tag" key={id}>{id}</code>)}</div></article>)}</div></section>}
    {presentation.improvement?.proposal?.candidatePolicyPatch && <section className="panel"><span className="eyebrow">Bounded revision surface</span><h2>Candidate policy patch</h2><p className="panel-note">The exact, scoped change the improvement proposes — the only surface the loop is allowed to touch.</p><JsonView value={presentation.improvement.proposal.candidatePolicyPatch} /><RawDrawer label="Raw policy patch JSON" value={presentation.improvement.proposal.candidatePolicyPatch} /></section>}
    {presentation.improvement?.policyEvaluation && <section className={presentation.improvement.policyEvaluation.passed ? "notice" : "notice notice-error"}><strong>Citation and candidate policy: {presentation.improvement.policyEvaluation.passed ? "passed" : "failed"}</strong><p>{presentation.improvement.policyEvaluation.message}</p></section>}
