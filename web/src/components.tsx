...
export function OperationTrace({ operation }: { operation: Operation }) {
  ...
  return (
    <section className="trace-panel" aria-live="polite">
      ...
      {artifactId && <Link className="button button-secondary" to={`/runs/${artifactId}`}>{operation.kind === "agent-improvement" ? "Inspect improvement proposal" : operation.kind === "agent-candidate-evaluation" ? "Inspect candidate comparison" : "Inspect persisted evidence"}</Link>}
      {evaluation && (
